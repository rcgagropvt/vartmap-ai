...
// ===== END ADMIN FINANCE =====
  ...
  app.get("/api/v1/farmers/:id/full-profile", auth, async (req, res) => {
    try {
      const { id } = req.params;
      const { rows: fr } = await pool.query("SELECT * FROM farmers WHERE id=", [id]);
      if (!fr[0]) return res.status(404).json({ error: "Farmer not found" });
      const farmer = fr[0];
      const msgs = await pool.query("SELECT * FROM wa_messages WHERE farmer_id= ORDER BY created_at DESC LIMIT 50", [id]).then(r=>r.rows).catch(()=>[]);
      const posts = await pool.query("SELECT * FROM community_posts WHERE farmer_id= ORDER BY created_at DESC LIMIT 20", [id]).then(r=>r.rows).catch(()=>[]);
      const diagnoses = await pool.query("SELECT * FROM wa_messages WHERE farmer_id= AND (content ILIKE '%diagnos%' OR content ILIKE '%disease%' OR content ILIKE '%pest%' OR content ILIKE '%treatment%') ORDER BY created_at DESC LIMIT 20", [id]).then(r=>r.rows).catch(()=>[]);
      const txns = await pool.query("SELECT * FROM farm_transactions WHERE farmer_id= ORDER BY date DESC LIMIT 50", [id]).then(r=>r.rows).catch(()=>[]);
      ...
      const expByCat = await pool.query("SELECT category, SUM(amount) as total FROM farm_transactions WHERE farmer_id= AND type='expense' GROUP BY category ORDER BY total DESC LIMIT 10", [id]).then(r=>r.rows).catch(()=>[]);
      const loyalty = await pool.query("SELECT * FROM farmer_loyalty WHERE farmer_id=", [id]).then(r=>r.rows[0]||null).catch(()=>null);
      const spins = await pool.query("SELECT * FROM spin_results WHERE farmer_id= ORDER BY created_at DESC LIMIT 10", [id]).then(r=>r.rows).catch(()=>[]);
      const iStats = await pool.query("SELECT COUNT(*) as total_messages, COUNT(CASE WHEN direction='inbound' THEN 1 END) as inbound, COUNT(CASE WHEN direction='outbound' THEN 1 END) as outbound FROM wa_messages WHERE farmer_id=", [id]).then(r=>r.rows[0]).catch(()=>({total_messages:0,inbound:0,outbound:0}));
      const reels = await pool.query("SELECT * FROM reels WHERE farmer_id= ORDER BY created_at DESC LIMIT 10", [id]).then(r=>r.rows).catch(()=>[]);
  ...
  app.post("/api/v1/farmers/:id/ai-summary", auth, async (req, res) => {
    try {
      ...
      const prompt = "You are an agricultural analyst for VartMap (Indian agri-tech). Analyze this farmer and give a 4-5 paragraph summary covering: 1)Overview 2)Farming 3)Finances 4)Communication 5)Recommendations. Under 300 words. Data: " + JSON.stringify(profileData);
